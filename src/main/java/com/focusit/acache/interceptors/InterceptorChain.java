...
/**
 * Knows how to build and manage an chain of interceptors. Also in charge with
 * invoking methods on the chain.
 *
...
public class InterceptorChain {
	 ...
	 *             if the position is invalid (e.g. 5 and there are only 2
	...
	/**
	 * Returns an unmofiable list with all the interceptors in sequence. If
	 * first in chain is null an empty list is returned.
	 */
	 ...
	 *
	 * @return true if the interceptor was added; i.e. the afterInterceptor
	 *         exists
	 */
	...
	/**
	 * @deprecated Use
	 *             {@link #addInterceptorBefore(org.infinispan.interceptors.base.CommandInterceptor, Class)}
	 *             instead.
	 */
	@Deprecated
	...
	/**
	 * Replaces an existing interceptor of the given type in the interceptor
	 * chain with a new interceptor instance passed as parameter.
	 *
	 * @param replacingInterceptor
	 *            the interceptor to add to the interceptor chain
	 * @param toBeReplacedInterceptorType
	 *            the type of interceptor that should be swapped with the new
	 *            one
	...
	public void appendInterceptor(CommandInterceptor ci, boolean isCustom) {
		...
		if (isCustom)
			validateCustomInterceptor(interceptorClass);
		assertNotAdded(interceptorClass);
		// Called when building interceptor chain and so concurrent start calls
	...
	/**
	 * Walks the command through the interceptor chain. The received ctx is
	 * being passed in.
	 */
	...
	/**
	 * Mainly used by unit tests to replace the interceptor chain with the
	 * starting point passed in.
	 *
	 * @param interceptor
	...
	/**
	 * Returns all the interceptors that have the fully qualified name of their
	 * class equal with the supplied class name.
	 */
