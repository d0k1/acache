...
import static com.focusit.acache.container.entries.ReadCommittedEntry.Flags.*;

/**
 * A wrapper around a cached entry that encapsulates read committed semantics
 * when writes are initiated, committed or rolled back.
 *
 * @author Manik Surtani (<a href="mailto:manik@jboss.org">manik@jboss.org</a>)
 * @since 4.0
 */
public class ReadCommittedEntry implements MVCCEntry {
	private static final Logger log = LoggerFactory.getLogger(ReadCommittedEntry.class);
	...
	// if this or any MVCC entry implementation ever needs to store a boolean,
	// always use a flag instead. This is far
	// more space-efficient. Note that this value will be stored in a byte,
	// which means up to 8 flags can be stored in
	// a single byte. Always start shifting with 0, the last shift cannot be
	// greater than 7.
	protected enum Flags {
		CHANGED(1 << 0), CREATED(1 << 1), REMOVED(1 << 2), VALID(1 << 3), EVICTED(1 << 4), EXPIRED(1 << 5), SKIP_LOOKUP(
				1 << 6), COPIED(1 << 7);
	...
	public final void commit(DataContainer container, Metadata providedMetadata) {
		// TODO: No tombstones for now!! I'll only need them for an eventually
		// consistent cache
		...
		if (isChanged()) {
			if (trace)
				log.trace(String.format(
						"Updating entry (key=%s removed=%s valid=%s changed=%s created=%s value=%s metadata=%s, providedMetadata=%s)",
						Util.toStr(getKey()), isRemoved(), isValid(), isChanged(), isCreated(), Util.toStr(value),
						getMetadata(), providedMetadata));

			// // Ugh!
			// if (value instanceof AtomicHashMap) {
			// AtomicHashMap<?, ?> ahm = (AtomicHashMap<?, ?>) value;
			// // Removing commit() call should not be an issue.
			// // If marshalling is needed (clustering, or cache store),
			// // calling
			// // delta() will clear the delta, avoiding leaking values in
			// // delta.
	...
	private Object compact(Object val) {
		...
		// {
		// byte[] compactedArray =
		// Arrays.copyOfRange(marshalled.getRaw().getRaw(), 0,
		// marshalled.getRaw().size());
		// return new MarshalledValue(compactedArray, marshalled.hashCode(),
	...
	public boolean skipLookup() {
		// in read committed, it can read from the data container / remote
	...
	public String toString() {
		return getClass().getSimpleName() + "(" + Util.hexIdHashCode(this) + "){" + "key=" + Util.toStr(key)
				+ ", value=" + Util.toStr(value) + ", oldValue=" + Util.toStr(oldValue) + ", isCreated=" + isCreated()
				+ ", isChanged=" + isChanged() + ", isRemoved=" + isRemoved() + ", isValid=" + isValid()
				+ ", isExpired=" + isExpired() + ", skipRemoteGet=" + skipLookup() + ", metadata=" + metadata + '}';
	}
