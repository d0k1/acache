 ...
 *
 * In order to provide linked list behaviour, entries are linked with each other
...
public class EquivalentLinkedHashMap<K, V> extends EquivalentHashMap<K, V> {
	...
	public EquivalentLinkedHashMap(int initialCapacity, float loadFactor, IterationOrder iterationOrder,
	...
	private abstract class EquivalentLinkedHashIterator<T> implements Iterator<T> {
		...
		protected EquivalentLinkedHashIterator(EquivalentHashMap<K, V> map) {
			this.map = map;
			nextEntry = new LinkedEntry<K, V>(header.after.key, header.after.value, header.after.before,
					header.after.after, map);
		}
		...
		boolean equals(LinkedEntry<K, V> entry, LinkedNode<K, V> node) {
			return entry.key == node.key && entry.val == node.value && entry.before == node.before
