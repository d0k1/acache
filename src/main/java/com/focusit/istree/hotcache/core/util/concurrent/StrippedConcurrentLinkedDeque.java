 ...
 */
/**
 * This is a subset of the java.util.concurrent.ConcurrentLinkedDeque to only
 * have methods for maintaining a doubly linked list but without maintaining the
 * java.util.Collection or java.util.Deque contracts. This is useful to keep a
 * linked list that will never be iterated upon but instead will have references
 * to elements directly in the list that will be removed or moved as needed.
 * 
 * @author wburns
 * @since 7.1
 */
...
class StrippedConcurrentLinkedDeque<E> {
	...
	static class DequeNode<E> {
		...
		DequeNode() { // default constructor for NEXT_TERMINATOR,
						// PREV_TERMINATOR
		}
	...
	/**
	 * Returns whether a node was polled or not. If a node was polled then the
	 * provided array will contain the Node that was removed (with it's item set
	 * to null) in the 0th position of the array and the item value in the 1st
	 ...
	 * 
	 * @return whether this node can be reused - A node may not be reused if it
	 *         was a head or tail because it can still have valid references to
	 *         it
	 */
