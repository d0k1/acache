...
public class InfinispanCollections {
	...
	public static interface Function<E, T> {
		...
		/**
		 * Transforms an instance of the given input into an instace of the type
		 * to be returned.
		 *
		 * @param input
	 ...
	 *
	 * @param input
	 *            contains the input key/value pair map
	 * @param f
	 *            function instance to use to transform the value part of the
	 *            map
	 * @param <K>
	 *            input map's key type
	 * @param <V>
	 *            desired output type of the map's value
	 * @param <E>
	 *            input map's value type
	...
	public static <K, V, E> Map<K, V> transformMapValue(Map<K, E> input, Function<E, V> f) {
		// This screams for a map function! Gimme functional programming
		// pleasee...
		if (input.isEmpty())
	 ...
	 * {@link MapMakerFunction}
	 *
	 * @param input
	 *            contains a collection of type E
	 * @param f
	 *            MapMakerFunction instance to use to transform the collection
	 *            to a key/value pair
	 * @param <K>
	 *            output map's key type
	 * @param <V>
	 *            output type of the map's value
	 * @param <E>
	 *            input collection's entry type
	...
	public static <K, V, E> Map<K, V> transformCollectionToMap(Collection<? extends E> input,
			MapMakerFunction<K, V, ? super E> f) {
		// This screams for a map function! Gimme functional programming
		// pleasee...
		if (input.isEmpty())
			return InfinispanCollections.emptyMap();
	...
	/**
	 * Returns the elements that are present in s1 but which are not present in
	 * s2, without changing the contents of neither s1, nor s2.
	 *
	 * @param s1
	 *            first set
	 * @param s2
	...
	/**
	 * Returns the empty set (immutable). Contrary to
	 * {@link Collections#emptySet}, the set returned returns a constant
	 * Iterator, rather than create a brand new one in each iterator call.
	 *
	...
	/**
	 * Returns the empty map (immutable). Contrary to
	 * {@link Collections#emptyMap()}, the map returned returns a constant
	 * Iterator, rather than create a brand new one in each iterator call.
	 *
	 * This set is marshallable using Infinispan's
	 * {@link org.jboss.marshalling.Externalizer} framework.
	...
	/**
	 * Returns the empty list (immutable). Contrary to
	 * {@link Collections#emptyList()}}, the list returned returns a constant
	 * Iterator, rather than create a brand new one in each iterator call.
	 *
	 * This set is marshallable using Infinispan's
	 * {@link org.jboss.marshalling.Externalizer} framework.
