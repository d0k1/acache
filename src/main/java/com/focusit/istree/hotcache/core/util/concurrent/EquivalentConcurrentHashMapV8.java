 ...
 * (using {@link #newKeySet(Equivalence)} or
 * {@link #newKeySet(int, Equivalence)}), or viewed (using
...
public class EquivalentConcurrentHashMapV8<K, V> extends AbstractMap<K, V>implements ConcurrentMap<K, V>, Serializable {
	...
	static class Node<K, V> implements Map.Entry<K, V> {
		...
		public final boolean equals(Object o) {
			...
			return ((o instanceof Map.Entry) && (k = (e = (Map.Entry<?, ?>) o).getKey()) != null
					&& (v = e.getValue()) != null && (k == key || nodeEq.keyEq.equals(key, k)) && // EQUIVALENCE_MOD
	 ...
	 */
	public EquivalentConcurrentHashMapV8(Equivalence<? super K> keyEquivalence,
	 ...
	 */
	public EquivalentConcurrentHashMapV8(int initialCapacity, Equivalence<? super K> keyEquivalence,
			Equivalence<? super V> valueEquivalence) {
	 ...
	 *            the map
	 */
	public EquivalentConcurrentHashMapV8(Map<? extends K, ? extends V> m, Equivalence<? super K> keyEquivalence,
			Equivalence<? super V> valueEquivalence) {
		this(keyEquivalence, valueEquivalence); // EQUIVALENCE_MOD
	 ...
	 * @since 1.6
	 */
	public EquivalentConcurrentHashMapV8(int initialCapacity, float loadFactor, Equivalence<? super K> keyEquivalence,
			Equivalence<? super V> valueEquivalence) {
	 ...
	 */
	public EquivalentConcurrentHashMapV8(int initialCapacity, float loadFactor, int concurrencyLevel,
	...
	final V putVal(K key, V value, boolean onlyIfAbsent) {
		...
		for (Node<K, V>[] tab = table;;) {
			...
			if (tab == null || (n = tab.length) == 0)
				...
				if (casTabAt(tab, i, null, new Node<K, V>(hash, nodeEq, key, value, null))) // EQUIVALENCE_MOD
					...
					if (first.hash < 0) {
						...
						if (insertAtFront && binCount >= TREEIFY_THRESHOLD) {
							...
							for (q = p; q != null; q = q.next) {
								TreeNode<K, V> t = new TreeNode<K, V>(q.hash, nodeEq, q.key, q.val, null, null); // EQUIVALENCE_MOD
								if ((t.prev = tl) == null)
									hd = t;
								else
									tl.next = t;
	...
	public static <K> KeySetView<K, Boolean> newKeySet(Equivalence<K> keyEquivalence) { // EQUIVALENCE_MOD
		return new KeySetView<K, Boolean>(
				new EquivalentConcurrentHashMapV8<K, Boolean>(keyEquivalence, AnyEquivalence.<Boolean> getInstance()),
				Boolean.TRUE); // EQUIVALENCE_MOD
	}
	...
	public static <K> KeySetView<K, Boolean> newKeySet(int initialCapacity, Equivalence<K> keyEquivalence) { // EQUIVALENCE_MOD
		return new KeySetView<K, Boolean>(new EquivalentConcurrentHashMapV8<K, Boolean>(initialCapacity, keyEquivalence,
					...
					if (tabAt(tab, i) == f) {
						...
						if (fh >= 0) {
							...
							for (Node<K, V> e = t.first; e != null; e = e.next) {
								int h = e.hash;
								TreeNode<K, V> p = new TreeNode<K, V>(h, nodeEq, e.key, e.val, null, null); // EQUIVALENCE_MOD
								if ((h & n) == 0) {
							...
							ln = (lc <= UNTREEIFY_THRESHOLD) ? untreeify(lo, nodeEq)
									: // EQUIVALENCE_MOD
									(hc != 0) ? new TreeBin<K, V>(lo, nodeEq) : t; // EQUIVALENCE_MOD
							hn = (hc <= UNTREEIFY_THRESHOLD) ? untreeify(hi, nodeEq)
									: // EQUIVALENCE_MOD
				...
				synchronized (b) {
					if (tabAt(tab, index) == b) {
						...
						for (Node<K, V> e = b; e != null; e = e.next) {
							TreeNode<K, V> p = new TreeNode<K, V>(e.hash, nodeEq, e.key, e.val, // EQUIVALENCE_MOD
									null, null);
							if ((p.prev = tl) == null)
								hd = p;
							else
	...
	static class BaseIterator<K, V> extends Traverser<K, V> {
		...
		BaseIterator(Node<K, V>[] tab, int size, int index, int limit, EquivalentConcurrentHashMapV8<K, V> map) {
	...
	static final class KeyIterator<K, V> extends BaseIterator<K, V>implements Iterator<K>, Enumeration<K> {
		KeyIterator(Node<K, V>[] tab, int index, int size, int limit, EquivalentConcurrentHashMapV8<K, V> map) {
	...
	static final class ValueIterator<K, V> extends BaseIterator<K, V>implements Iterator<V>, Enumeration<V> {
		ValueIterator(Node<K, V>[] tab, int index, int size, int limit, EquivalentConcurrentHashMapV8<K, V> map) {
			super(tab, index, size, limit, map);
		}
	...
	static final class EntryIterator<K, V> extends BaseIterator<K, V>implements Iterator<Map.Entry<K, V>> {
		EntryIterator(Node<K, V>[] tab, int index, int size, int limit, EquivalentConcurrentHashMapV8<K, V> map) {
			super(tab, index, size, limit, map);
		}
	...
	abstract static class CollectionView<K, V, E> implements Collection<E>, java.io.Serializable {
		...
		CollectionView(EquivalentConcurrentHashMapV8<K, V> map) {
			this.map = map;
		}
	...
	static final class ValuesView<K, V> extends CollectionView<K, V, V>implements Collection<V>, java.io.Serializable {
		...
		ValuesView(EquivalentConcurrentHashMapV8<K, V> map) {
			super(map);
		}
	...
	static final class EntrySetView<K, V> extends CollectionView<K, V, Map.Entry<K, V>>
		...
		EntrySetView(EquivalentConcurrentHashMapV8<K, V> map) {
			super(map);
		}
	...
	static final class MapReduceEntriesToDoubleTask<K, V> extends BulkTask<K, V, Double> {
		...
		MapReduceEntriesToDoubleTask(BulkTask<K, V, ?> p, int b, int i, int f, Node<K, V>[] t,
				MapReduceEntriesToDoubleTask<K, V> nextRight, ToDoubleFunction<Entry<K, V>> transformer, double basis,
				DoubleBinaryOperator reducer) {
			super(p, b, i, f, t);
			this.nextRight = nextRight;
			this.transformer = transformer;
	...
	static final class MapReduceEntriesToLongTask<K, V> extends BulkTask<K, V, Long> {
		...
		MapReduceEntriesToLongTask(BulkTask<K, V, ?> p, int b, int i, int f, Node<K, V>[] t,
				MapReduceEntriesToLongTask<K, V> nextRight, ToLongFunction<Entry<K, V>> transformer, long basis,
				LongBinaryOperator reducer) {
			super(p, b, i, f, t);
			this.nextRight = nextRight;
			this.transformer = transformer;
	...
	static final class MapReduceEntriesToIntTask<K, V> extends BulkTask<K, V, Integer> {
		...
		MapReduceEntriesToIntTask(BulkTask<K, V, ?> p, int b, int i, int f, Node<K, V>[] t,
				MapReduceEntriesToIntTask<K, V> nextRight, ToIntFunction<Entry<K, V>> transformer, int basis,
				IntBinaryOperator reducer) {
			super(p, b, i, f, t);
			this.nextRight = nextRight;
			this.transformer = transformer;
