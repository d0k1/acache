...
import com.focusit.istree.hotcache.core.util.AbstractMap;
 ...
 * implementations for keys and values, as opposed to relying on their own
 ...
 *
 * This hash map implementation is optimised for store/retrieval rather than
 * iteration. Internal node entries are not linked, so responsibility to link
 * them falls on the iterators.
 *
...
public class EquivalentHashMap<K, V> extends AbstractMap<K, V> {
	...
	public EquivalentHashMap(int initialCapacity, float loadFactor, Equivalence<? super K> keyEq,
	...
	@SuppressWarnings("unchecked")
	public EquivalentHashMap(Map<? extends K, ? extends V> map, Equivalence<? super K> keyEq,
			Equivalence<? super V> valueEq) {
	...
	static class MapEntry<K, V> implements Map.Entry<K, V> {
		...
		public final String toString() {
			return map.keyEq.toString(key) + "=" + map.valueEq.toString(val);
		}
		...
		public final boolean equals(Object o) {
			Object k, v;
			Entry<?, ?> e;
			return ((o instanceof Entry) && (k = (e = (Entry<?, ?>) o).getKey()) != null && (v = e.getValue()) != null
					&& (k == key || map.keyEq.equals(key, k)) && (v == val || map.valueEq.equals(val, v)));
		}
	}
