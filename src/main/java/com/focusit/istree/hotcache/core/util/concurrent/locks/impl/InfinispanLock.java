package com.focusit.istree.hotcache.core.util.concurrent.locks.impl;
...
import com.focusit.istree.hotcache.core.util.TimeService;
import com.focusit.istree.hotcache.core.util.concurrent.CompletableFutures;
import com.focusit.istree.hotcache.core.util.concurrent.TimeoutException;
import com.focusit.istree.hotcache.core.util.concurrent.locks.DeadlockChecker;
import com.focusit.istree.hotcache.core.util.concurrent.locks.DeadlockDetectedException;
import com.focusit.istree.hotcache.core.util.concurrent.locks.ExtendedLockPromise;
import com.focusit.istree.hotcache.core.util.concurrent.locks.LockListener;
import com.focusit.istree.hotcache.core.util.concurrent.locks.LockState;
...
public class InfinispanLock {
   ...
   private static final Logger log = LoggerFactory.getLogger(InfinispanLock.class);
   ...
   public ExtendedLockPromise acquire(Object lockOwner, long time, TimeUnit timeUnit) {
      ...
      if (trace) {
         log.trace(String.format("Acquire lock for %s. Timeout=%s (%s)", lockOwner, time, timeUnit));
      }

      LockPlaceHolder lockPlaceHolder = lockOwners.get(lockOwner);
      if (lockPlaceHolder != null) {
         if (trace) {
            log.trace(String.format("Lock owner already exists: %s", lockPlaceHolder));
         }
      ...
      if (other != null) {
         if (trace) {
            log.trace(String.format("Lock owner already exists: %s", other));
         }
         return other;
      }

      if (trace) {
         log.trace(String.format("Created a new one: %s", lockPlaceHolder));
      }
   ...
   public void release(Object lockOwner) {
      ...
      if (trace) {
         log.trace(String.format("Release lock for %s.", lockOwner));
      }

      LockPlaceHolder wantToRelease = lockOwners.get(lockOwner);
      if (wantToRelease == null) {
         if (trace) {
            log.trace(String.format("%s not found!", lockOwner));
         }
         //nothing to release
         return;
      }
      ...
      if (trace) {
         log.trace(String.format("Release lock for %s? %s", wantToRelease, released));
      }
   ...
   private void onCanceled(LockPlaceHolder canceled) {
      if (trace) {
         log.trace(String.format("Release lock for %s. It was canceled.", canceled.owner));
      }
      LockPlaceHolder currentLocked = current;
   ...
   private void tryAcquire(LockPlaceHolder release) {
      ...
      do {
         ...
         if (trace) {
            log.trace(String.format("Try acquire. Next in queue=%s. Current=%s", toAcquire, current));
         }
         ...
         if (cas(toRelease, toAcquire)) {
            ...
            if (toAcquire.setAcquire()) {
               if (trace) {
                  log.trace(String.format("%s successfully acquired the lock.", toAcquire));
               }
               return;
            }
            if (trace) {
               log.trace(String.format("%s failed to acquire (invalid state). Retrying.", toAcquire));
            }
            //oh oh, probably the nextPending Timed-Out. we are going to retry with the next in queue
            toRelease = toAcquire;
         } else {
            if (trace) {
               log.trace(String.format("Unable to acquire. Lock is held."));
            }
   ...
   private class LockPlaceHolder implements ExtendedLockPromise {
      ...
      public void lock() throws InterruptedException, TimeoutException {
         while (true) {
            switch (lockState) {
               case WAITING:
                  checkTimeout();
                  CompletableFutures.await(notifier, timeService.remainingTime(timeout, TimeUnit.MILLISECONDS), TimeUnit.MILLISECONDS);
                  break;
               case ACQUIRED:
                  return; //acquired!
               case RELEASED:
      ...
      private boolean casState(LockState expect, LockState update) {
         ...
         if (updated && trace) {
            log.trace(String.format("State changed for %s. %s => %s", this, expect, update));
         }
         return updated;
      }
