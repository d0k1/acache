package com.focusit.istree.hotcache.core.transaction.impl;
...
import com.focusit.istree.hotcache.core.commands.write.WriteCommand;
...
import com.focusit.istree.hotcache.core.container.versioning.EntryVersionsMap;
import com.focusit.istree.hotcache.core.container.versioning.IncrementableEntryVersion;
import com.focusit.istree.hotcache.core.context.Flag;
import com.focusit.istree.hotcache.core.equivalence.Equivalence;
import com.focusit.istree.hotcache.core.transaction.xa.CacheTransaction;
import com.focusit.istree.hotcache.core.transaction.xa.GlobalTransaction;
import com.focusit.istree.hotcache.core.util.CollectionFactory;
import com.focusit.istree.hotcache.core.util.ImmutableListCopy;
import com.focusit.istree.hotcache.core.util.Immutables;
import com.focusit.istree.hotcache.core.util.InfinispanCollections;
import com.focusit.istree.hotcache.core.util.Util;
...
public abstract class AbstractCacheTransaction implements CacheTransaction {
   ...
   private static Logger log = LoggerFactory.getLogger(AbstractCacheTransaction.class);
   ...
   public void notifyOnTransactionFinished() {
      if (trace) log.trace(String.format("Transaction %s has completed, notifying listening threads.", tx));
      if (!txComplete) {
   ...
   public void registerLockedKey(Object key) {
      ...
      if (trace) log.trace(String.format("Registering locked key: %s", Util.toStr(key)));
      lockedKeys.add(key);
   }
   ...
   public void clearLockedKeys() {
      if (trace) log.trace(String.format("Clearing locked keys: %s", Util.toStr(lockedKeys)));
      lockedKeys = null;
   }
   ...
   public void addVersionRead(Object key, EntryVersion version) {
      ...
      if (!versionsSeenMap.containsKey(key)) {
         if (log.isTraceEnabled()) {
            log.trace(String.format("Transaction %s read %s with version %s", getGlobalTransaction().globalId(), key, version));
         }
   ...
   public void replaceVersionRead(Object key, EntryVersion version) {
      ...
      if (log.isTraceEnabled()) {
         log.trace(String.format("Transaction %s replaced version for key %s. old=%s, new=%s", getGlobalTransaction().globalId(), key,
                    oldVersion, version));
      }
   }
