...
import com.focusit.istree.hotcache.core.transaction.totalorder.TotalOrderLatch;
...
public class TotalOrderRemoteTransactionState {
   ...
   private static final Logger log = LoggerFactory.getLogger(TotalOrderRemoteTransactionState.class);
   ...
   public synchronized void prepared() {
      if (log.isTraceEnabled()) {
         log.trace(String.format("[%s] Current status is %s, setting status to: PREPARED", globalTransaction.globalId(),
                    transactionState));
      }
   ...
   public synchronized void preparing() {
      if (log.isTraceEnabled()) {
         log.trace(String.format("[%s] Current status is %s, setting status to: PREPARING", globalTransaction.globalId(),
                    transactionState));
      }
   ...
   public final synchronized boolean waitUntilPrepared(boolean commit)
      ...
      if (log.isTraceEnabled()) {
         log.trace(String.format("[%s] Current status is %s, setting status to: %s", globalTransaction.globalId(),
      ...
      if (transactionState.contains(State.PREPARED)) {
         result = true;
         if (log.isTraceEnabled()) {
            log.trace(String.format("[%s] Transaction is PREPARED", globalTransaction.globalId()));
         }
      } else if (transactionState.contains(State.PREPARING)) {
         wait();
         result = true;
         if (log.isTraceEnabled()) {
            log.trace(String.format("[%s] Transaction was in PREPARING state but now it is prepared", globalTransaction.globalId()));
         }
      } else {
         if (log.isTraceEnabled()) {
            log.trace(String.format("[%s] Transaction is not delivered yet", globalTransaction.globalId()));
         }
         result = false;
      }
      return result;
