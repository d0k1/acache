package com.focusit.istree.hotcache.core.equivalence;

import java.util.AbstractSet;
import java.util.Iterator;

/**
 * Custom hash-based set which accepts no null values, where equality and hash
 * code calculations are done based on passed
 * {@link org.infinispan.commons.equivalence.Equivalence} function
 * implementations for values, as opposed to relying on their own
 ...
 * where users want to avoid using wrapper objects.
 *
...
public class EquivalentHashSet<E> extends AbstractSet<E> {
	 ...
	 * functionality to check equality, calculate hash codes...etc of the stored
	 * entries.
	 */
	 ...
	 *
	 * @param entryEq
	 *            the Equivalence function to be used to compare entries in this
	 *            set.
	 */
	...
	public EquivalentHashSet(int initialCapacity, Equivalence<? super E> entryEq) {
		this.entryEq = entryEq;
		m = new EquivalentHashMap<E, Boolean>(initialCapacity, entryEq, AnyEquivalence.BOOLEAN);
	}
	 ...
	 * function associated with it. The hash code of a set is defined to be the
	 * sum of the hash codes of the elements in the set, where the hash code of
	 * a <tt>null</tt> element is defined to be zero. This ensures that
	 * {@link Equivalence#equals(Object s1, Object s2)} implies that
	 * {@link Equivalence#hashCode(Object s1)}==
	 * {@link Equivalence#hashCode(Object s2)} for any two sets <tt>s1</tt> and
